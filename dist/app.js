...
const primitiveTypes = new Set([
  "any", "bool", "char", "Code", "cstring", "cstring16", "c32", "c64", "c128", "c160", "c256",
...
function constraintsAllow(signature, bindings) {
  ...
  for (const match of where.matchAll(/\b(Is_Bool|Is_Char|Is_Character|Is_Complex|Is_Float|Is_Integer|Is_Numeric|Is_Pointer|Is_Procedure|Is_Rune|Is_Simd|Is_String)\s*\(\s*\$?([A-Z][A-Za-z0-9_]*)/g)) {
    const actual = bindings.get(match[2]);
    if (!actual) continue;
    if (match[1] === "Is_Bool" && actual.toLowerCase() !== "bool") return false;
    if (match[1] === "Is_Char" && actual.toLowerCase() !== "char") return false;
    if (match[1] === "Is_Character" && !/^(?:char|rune)$/i.test(actual)) return false;
